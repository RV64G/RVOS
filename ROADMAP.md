...
- **任务**: 将 `mm/page.c` 中的线性扫描算法替换为 **伙伴系统 (Buddy System)**。
    ...
    -   **分配逻辑 (`page_alloc`)**:
        -   根据请求的页数，计算出合适的阶。
        -   从对应阶的空闲链表中查找。如果为空，则向更高一阶查找，并将找到的大块“分裂”成两个伙伴，一个用于分配，另一个放入低阶的空-   **释放逻辑 (`page_free`)**:
        -   释放一个块时，计算其“伙伴”块的地址。
...
### 第六步：内核原语与优化 (Kernel Primitives and Optimization)

- **目标**: 提升内核底层原语的性能和可移植性。
- **任务**:
    1. **适配 Zihintpause 扩展**:
        - **目的**: 在支持该扩展的 RISC-V 处理器上，降低自旋锁等待时的功耗和资源竞争。
        - **实现**:
            - 在构建系统中添加编译开关 (e.g., `HAVE_ZIHINTPAUSE`)。
            - 在内核代码中实现一个 `spin_loop_hint()` 内联函数，根据编译开关选择使用 `pause` 指令或空汇编语句。
            - 在自旋锁的等待循环中调用 `spin_loop_hint()`。