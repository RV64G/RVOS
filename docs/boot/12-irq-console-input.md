...
PLIC 只负责外部中断控制器这一层。UART 自己也有中断开关。只有 UART 的 RX interrupt
和 PLIC 的 UART source 都打开，输入字符才会一路送到 S-mode external interrupt。
...
`uart_enable_rx_interrupt()` 打开 ns16550 的 `IER.RDI` 位。之后只要接收缓冲区里有字符，
UART 就会向 PLIC 拉起自己的 interrupt source。

`uart_handle_interrupt()` 不负责打印。它只在 `LSR.DR` 表示有数据时读取 `RBR`，并把
字符放进 console ring buffer：
...
这样做是为了让硬中断路径尽量短。打印会等待 UART 发送寄存器空闲，不适合直接放在
IRQ handler 里。
