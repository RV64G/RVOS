...
前者负责和固件交互、收集启动信息、退出 Boot Services；后者才是长期运行的内核。
当前 `kernel.elf` 还是最小入口，用来固定构建边界；EFI loader 已经会从 ESP 里读取
它、装载它，并在退出 Boot Services 后跳到它的入口。
...
这个文件应该放到 FAT32/ESP 分区的 UEFI 默认启动路径：
...
`boot/efi/boot/` 保存当前 EFI 应用的实现，`boot/efi/riscv/` 保存 EFI loader 私有的
RISC-V 代码。Makefile 会扫描这些目录下的 `.c` 和 `.S`，新增 EFI 启动源文件时不需要
手工维护对象列表。

需要生成能直接挂载启动的 ESP 镜像时运行：
...
EFI loader 读取内核 ELF 时不自己解析 FAT32，而是走 UEFI 提供的文件协议：
...
当前会校验 ELF64/RISC-V 文件头和 program header 表，确认文件存在、格式正确，然后
根据 `PT_LOAD` 段计算页对齐的物理装载区间，把段内容复制到 ELF 指定的 `p_paddr`。
装载完成后，EFI loader 会重新读取最终 memory map，退出 Boot Services，再跳到
ELF header 里的 `e_entry`。
...
memory map，读取并装载 `RVOS/KERNEL.ELF`，重新读取最终 memory map，立刻
`ExitBootServices`，切到初始内核栈，再 jump 到 kernel ELF 的入口。当前内核入口
会通过 SBI debug console 打印一行 `RVOS kernel ELF entered` 后停住。
...
从 EFI loader 跳到内核入口时不走普通 C 函数调用，而是走
`boot/efi/riscv/jump.S`。这里先把 `sp` 切到 EFI loader 准备的初始内核栈，再把
`boot_info` 放进 `a0`，最后用 `jr` 交出控制权。内核不应该返回 EFI loader。

RISC-V UEFI 固件可能在 S-mode 开着自己的页表。RVOS 的 kernel ELF 当前按物理地址
链接并装载，所以跳转前会清 `satp` 并执行 `sfence.vma`，让后续取指和访存回到
物理地址直通状态。

早期内置 stub 曾经用于验证“退出 EFI 后还能跑 C 代码”。外部 kernel ELF 接入后，
跳转目标直接来自 ELF header 里的入口地址。

## 共享的架构 ABI 代码

EFI loader 和内核都会用到一部分 RISC-V ABI 级代码，例如 SBI `ecall` 包装。这类代码
放在 `include/riscv/`，由两边直接包含。

这里不让 Makefile 在构建时创建源码软链接或硬链接。构建系统应该只决定“哪些源文件
参与哪个产物”，不应该临时改造源码树。需要共享的头文件放到公共 include 目录；如果
以后出现必须共享的 `.c` 文件，就放到明确的公共源码目录，然后分别编译到不同对象路径：
...
同一份源码分别编进 EFI loader 和 kernel 是正常的，因为它们是两个独立链接产物。真正
需要避免的是同一个最终产物里把同一个源文件扫进来两次。
...
EFI app 和内核已经拆成两个链接产物。旧 `os.ld` 只服务于 `make image` 这条旧裸镜像
路径；默认 EFI 路径里的内核使用 `kernel/kernel.lds`。
